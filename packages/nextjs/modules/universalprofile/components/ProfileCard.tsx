...
import { getImageUrl } from "~~/lukso/utils";
...
const ProfileCard = ({ profile }: any) => {
  ...
  return (
    <div className="flex items-center justify-center w-full h-full">
      ...
      >
        {hasBgImage ? (
          <Image
            ...
            src={getImageUrl(profile.backgroundImage) as string}
            alt={profile.name}
          />
        ) : null}
        <div
        ...
        >
          <Image
            ...
            src={getImageUrl(profile.profileImage) as string}
            alt={profile.name}
          />
        </div>
      ...
      <style jsx>{`
        #container {
          box-shadow: rgba(0, 0, 0, 0.06) 0px 1px 2px, rgba(0, 0, 0, 0.06) 0px 2px 4px, rgba(0, 0, 0, 0.06) 0px 4px 8px,
            rgba(0, 0, 0, 0.06) 0px 8px 16px, rgba(0, 0, 0, 0.06) 0px 16px 32px, rgba(0, 0, 0, 0.06) 0px 32px 64px;
        }
      `}</style>
    </div>
  );

...
import { ADDRESSES, PERMISSIONS, PERMISSIONS_ARRAY } from "~~/lukso/consts/main";
...
export const transferLXY = async (
  ...
  const { profileContract, keyManagerContract } = await createContractsInstance(profileAddress, web3);
...
export const addToPermissionsArray = async (
  ...
  const transaction = keyManagerContract.methods.execute(payload).send({ from: accountAddress, gasLimit: 3_000_000 });
